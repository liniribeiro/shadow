from alura.testing_classes.src.leilao.excecoes import LanceInvalido
...
class Usuario:
    ...
    def propoe_lance(self, leilao, valor):
        if not self._valor_eh_valido(valor):
            raise LanceInvalido("Não pode propor lance com o valor maior que valor da carteira")
...
class Leilao:
    ...
    @property
    def lances(self):
        # Retorna uma cópia de lista, é bom para garantir que comportamento nao esperado aconteça.
    ...
    def _usuarios_diferentes(self, lance):
        if self.__lances[-1].usuario != lance.usuario:
            return True
        raise LanceInvalido('Os usuários não podem dar dois lances seguidos')
